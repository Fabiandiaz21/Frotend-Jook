<template>
  <div class="q-pa-md">
    ...
    <q-card flat bordered class="q-mb-md q-pa-sm control-panel-card">
      <q-card-section>
        <div class="row q-col-gutter-md items-center">
          ...
          <div class="col-12 col-md-auto">
            <q-btn :label="showOfertasOnly ? 'Ver Todos los Productos' : 'Ver Ofertas'"
              :icon="showOfertasOnly ? 'store' : 'local_offer'" color="purple-8" @click="toggleOfertasFilter" glossy
              class="action-btn" />
          </div>
    ...
    <q-page class="q-pa-md bg-white">
      <div style="width: 100%; overflow-x: auto;">
        <q-table title="Lista de Productos" :rows="filteredProductos" :columns="columnas" row-key="_id"
          :loading="loadingTable" flat bordered wrap-cells class="my-product-table shadow-3 rounded-borders"
                ...
                <q-td v-else-if="col.name === 'acciones'" align="center">
                  <div class="row no-wrap justify-center q-gutter-xs">
                    ...
                    <q-btn :icon="props.row.estado === 'activo' ? 'cancel' : 'check'"
                      ...
                      <q-tooltip>
                        {{ props.row.estado === 'activo' ? 'Desactivar' : 'Activar' }} Producto
                      </q-tooltip>
                    </q-btn>
                    <q-btn v-if="!props.row.hasActiveOffer" icon="local_offer" color="teal" dense size="sm" flat
                      label="Oferta" @click="openOfertaDialog(props.row)" class="q-px-xs">
    ...
    <q-dialog v-model="editDialog" :persistent="false">
      <q-card style="min-width: 500px; max-width: 90vw; background-color: #fdf6ee;">
        <!-- Título -->
        <q-card-section class="row items-center justify-between" style="background-color: #6f4e37; color: white;">
        ...
        <q-card-section>
          <q-form @submit.prevent="handleEditProduct">
            <q-input v-model="productToedit.nombre" label="Nombre del Producto" outlined dense required color="brown-8"
              label-color="brown-7" />
            ...
            <q-select outlined dense v-model="productToedit.marca" :options="allMarcas" label="Marca" clearable
              ...
              new-value-mode="add-unique" emit-value map-options option-value="_id" option-label="label" color="brown-8"
              label-color="brown-7">
              <template v-slot:no-option>
                <q-item>
              ...
              <template v-slot:option="scope">
                <q-item v-bind="scope.itemProps" class="bg-brown-1 text-brown-10">
                  <q-item-section avatar v-if="scope.opt.imagen">
                    <q-avatar rounded size="32px">
                      <img :src="scope.opt.imagen">
                    </q-avatar>
            ...
            <!-- Vista previa de imagen de marca -->
            <div class="q-mt-sm" v-if="marcaImagePreviewEdit">
              <div class="flex flex-center">
                <img :src="marcaImagePreviewEdit" alt="Marca Preview"
                  style="max-width: 150px; max-height: 150px; object-fit: cover; border-radius: 8px; border: 2px solid #a0522d;" />
              </div>
            </div>
            ...
            <q-select outlined dense v-model="productToedit.tipo" :options="allTipos" label="Tipo" clearable use-input
              ...
              emit-value map-options option-value="_id" option-label="label" color="brown-8" label-color="brown-7">
              <template v-slot:no-option>
                <q-item>
                  <q-item-section class="text-grey-7">
            ...
            <!-- Descripción -->
            <q-input v-model="productToedit.descripcion" label="Descripción" outlined dense required color="brown-8"
              label-color="brown-7" />

            <!-- Precio -->
            <q-input v-model="productToedit.price" label="Precio" type="number" outlined dense required color="brown-8"
              label-color="brown-7" />

            <!-- Categoría -->
            <q-select v-model="productToedit.categoryId" :options="categories" label="Categoría" outlined dense
              emit-value map-options option-value="_id" option-label="nombre" required color="brown-8"
              label-color="brown-7" />

            <!-- Stock -->
            <q-input v-model="productToedit.stock" label="Stock" type="number" outlined dense required color="brown-8"
              label-color="brown-7" />

            <!-- Subida de imágenes -->
            <div class="q-mt-md">
              <label class="text-subtitle2 text-brown-8">Cambiar imágenes del producto (máx. 4):</label>
              <div class="q-gutter-sm row justify-center q-mt-sm">
                <div v-for="n in 4" :key="'edit-' + n">
                  <q-btn :label="'Imagen ' + n" color="brown-7" icon="cloud_upload"
                    @click="$refs['editImageInput' + n][0].click()" unelevated rounded class="text-white" />
              ...
              <div class="q-gutter-sm q-mt-sm">
                <div class="q-mt-sm" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
                  <div v-for="(img, i) in editImagePreviews" :key="i" class="flex flex-column items-center">
                    <!-- Etiqueta de número -->
                    <div class="text-subtitle2 text-brown-8 q-mb-xs">Imagen {{ i + 1 }}</div>

                    <div class="relative-position" style="width: 150px; height: 150px;">
                      <img :src="img" class="q-img rounded-borders shadow-2"
                        style="width: 150px; height: 150px; object-fit: cover; border-radius: 8px; border: 1px solid #8b4513;" />
                      <q-btn round dense flat size="xs" icon="close" color="red-6" class="absolute"
                        style="top: 4px; right: 4px; background-color: rgba(255,255,255,0.8); width: 24px; height: 24px; min-width: 0;"
            ...
            <!-- Botón guardar -->
            <q-btn type="submit" label="Guardar Cambios" color="brown-9" class="q-mt-md" :loading="loading" unelevated
              block />
          </q-form>
        </q-card-section>
      </q-card>
      ...
      <q-card style="min-width: 500px; max-width: 90vw; background-color: #fdf6ee;">
        ...
        <q-card-section>
          <q-form @submit.prevent="handleAddProduct">
            <q-input v-model="productName" label="Nombre del Producto" outlined dense required color="brown-8"
              label-color="brown-7" />
            ...
            <q-select outlined dense v-model="productMarca" :options="allMarcas" label="Marca" clearable use-input
              ...
              emit-value map-options color="brown-8" label-color="brown-7">
              <template v-slot:no-option>
                <q-item>
            ...
            <q-input v-model="productDescription" label="Descripción" outlined dense required color="brown-8"
              label-color="brown-7" />

            <q-input v-model="productPrice" label="Precio" type="number" outlined dense required color="brown-8"
              label-color="brown-7" />

            <q-select v-model="categoryId" :options="categories" label="Categoría" outlined dense emit-value map-options
              required color="brown-8" label-color="brown-7" />

            <q-input v-model="productStock" label="Stock" type="number" outlined dense required color="brown-8"
              label-color="brown-7" />

            <!-- Subida de imágenes -->
            <div class="q-mt-sm">
              <label class="text-subtitle2 text-brown-8 q-mb-sm">Seleccionar imágenes del producto (hasta 4):</label>

              <div class="q-gutter-sm row justify-center q-mt-sm">
                <div v-for="n in 4" :key="n">
                  <q-btn :label="'Imagen ' + n" color="brown-7" icon="cloud_upload"
                    @click="$refs['imageInput' + n][0].click()" flat rounded class="text-white" />
              ...
              <div class="q-mt-sm" style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 16px;">
                <div v-for="(img, i) in imagePreviews" :key="i" class="flex flex-column items-center">
                  <div class="text-subtitle2 text-brown-8 q-mb-xs">Imagen {{ i + 1 }}</div>
                  <div class="relative-position" style="width: 150px; height: 150px;">
                    <img :src="img" class="q-img rounded-borders shadow-2"
                      style="width: 150px; height: 150px; object-fit: cover; border-radius: 8px; border: 1px solid #8b4513;" />
                    <q-btn round dense flat size="xs" icon="close" color="red-6" class="absolute"
                      style="top: 4px; right: 4px; background-color: rgba(255,255,255,0.85); width: 24px; height: 24px; min-width: 0;"
                      @click="removeImage(i)" />
                  </div>
                </div>
              </div>
            ...
            <!-- Botón -->
            <q-btn type="submit" label="Agregar Producto" color="brown-9" class="q-mt-md" :loading="loading" unelevated
              block />
          </q-form>
        </q-card-section>
      </q-card>
    ...
    <q-dialog v-model="addMarcaDialog" :persistent="false">
      <q-card style="min-width: 600px; max-width: 90vw; background-color: #fdf6ee;">
        <!-- Encabezado -->
        <q-card-section class="row items-center justify-between" style="background-color: #6f4e37; color: white;">
        ...
        <q-card-section class="q-pt-none">
          <q-input outlined dense v-model="newMarcaName" label="Nombre de la Marca" required color="brown-8"
            label-color="brown-7" />

          <q-file outlined dense v-model="newMarcaImageFile" label="Imagen de la Marca (opcional)" accept="image/*"
            color="brown-8" label-color="brown-7" class="q-mt-md">
        ...
        <q-card-actions align="right">
          <q-btn flat label="Cancelar" color="brown-5" v-close-popup />
          <q-btn label="Guardar" color="brown-9" @click="handleAddNewMarca" :loading="loading" />
        </q-card-actions>
      </q-card>
    </q-dialog>
    ...
    <<q-dialog v-model="addTipoDialog" :persistent="false">
      <q-card style="min-width: 600px; background-color: #fdf6ee;">
        <!-- Encabezado -->
        <q-card-section class="row items-center justify-between" style="background-color: #6f4e37; color: white;">
        ...
        <!-- Campo de nombre -->
        <q-card-section class="q-pt-none">
          <q-input outlined dense v-model="newTipoName" label="Nombre del Tipo" required color="brown-8"
            label-color="brown-7" />
        </q-card-section>
        ...
        <q-card-actions align="right">
          <q-btn flat label="Cancelar" color="brown-5" v-close-popup />
          <q-btn label="Guardar" color="brown-9" @click="handleAddNewTipo" :loading="loading" />
        </q-card-actions>
      </q-card>
      </q-dialog>
        ...
        <q-card style="min-width: 400px; max-width: 90vw;">
          ...
          <q-card-section class="q-pt-none">
            <div v-if="offerToEdit">
              ...
              <q-input outlined dense v-model="offerPercentage" label="Porcentaje de Descuento (%)" type="number"
                min="1" max="100" required class="q-mt-md" />
  ...
  .control-panel-card {
    background-color: #f7f2ed;
    /* Un café muy claro para el fondo del panel */
    border-color: #d7ccc8;
  ...
  .custom-input .q-field__control {
    ...
    /* Fondo ligeramente crema dentro del input/select */
  }
  ...
  .custom-input .q-icon {
    color: #5d4037;
    /* Color café para los íconos de búsqueda/flecha */
  }

  /* Ajustes específicos para el q-select con imágenes en opciones */
  .q-item-section img {
    border-radius: 4px;
    /* Bordes redondeados para las imágenes de marca en las opciones */
  }
  ...
  .image-preview-card {
    height: 190px;
    /* Asegura una altura consistente para las previsualizaciones */
    display: flex;
  ...
  .image-preview-card {
    height: 190px;
    /* Altura consistente para las tarjetas de imagen */
    display: flex;
    flex-direction: column;
    justify-content: space-between;
  }
